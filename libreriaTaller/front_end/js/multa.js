...
function listarPrestamo() {
    ...
    $.ajax({
      ...
      success: function (result) {//success: funcion que se ejecuta cusndo la peticion tiene exito
        console.log(result);
        let curpoTablaMulta = document.getElementById("curpoTablaMulta");
        curpoTablaMulta.innerHTML = "";
        for (let i = 0; i < result.length; i++) {
          //se crea una etiqueta tr por cada registroz
          ...
          celdaFecha_devolucion.innerText = result[i]["fecha_prestamo"];
          celdaUsuarioMultado.innerText = result[i]["fecha_devolucion"];
          celdaPrestamo.innerText = result[i]["Estado"];
          celdaValorMulta.innerText = result[i]["usuario_prestamo"];
          celdaEstadoMulta.innerText = result[i]["libro_prestamo"];
